...
var
	...
	MatchState = require('./match-state');
...
class Node {
	...
	match(string, match_state) {
		if (!match_state) {
			match_state = new MatchState();
		}

		if (this.hasRuleName()) {
			return this.matchViaRuleName(string, match_state);
		}

		if (this.hasQuotedString()) {
			return this.matchViaQuotedString(string);
		}

		if (this.hasNumericValue()) {
			return this.matchViaNumericValue(string);
		}

		if (this.hasChildNodes()) {
			return this.matchViaChildNodes(string, match_state);
		}

		throw new Error('implement');
	}

	matchViaRuleName(string, match_state) {
		var
	...
	matchViaChildNodes(string, match_state) {
		if (this.hasAlternatives()) {
