...
class Node {
	...
	toJSON(recursive = false, seen_nodes) {
		...
		if (this.isRepeating()) {
			...
			// Serialize Infinity as a string;
			// otherwise it will be represented as null.
			if (this.max_repeats === Infinity) {
				result.max_repeats = 'Infinity';
			} else {
		...
		if (this.hasChildNodes() && recursive) {
			...
			result.child_node_count = this.getChildNodes().length;
		}
