...
class Unparser {
	...
	createCallback(callback_or_map) {
		if (!callback_or_map) {
			throw new Error('Invalid argument supplied to unparse()');
		}
		...
		function getValueForRule(rule_name) {
			...
			return callback_or_map(rule_name, count);
		}
	...
	unparseViaRuleName(callback) {
		...
		if (!rule_value) {
			throw new MissingRuleValueError(rule_name, null);
		}

		var match = this.getRule().match(rule_value);

		if (!match) {
			throw new InvalidRuleValueError(rule_name, rule_value);
		}
