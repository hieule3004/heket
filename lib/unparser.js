...
/**
 * Just a simple caching mechanism so we don't spuriously create duplicate
 * unparser instances for the same node over and over.
 *
 * @param   {lib/node} node
 * @returns {lib/unparser}
 */
function getUnparserForNode(node) {
	var node_id = node.getId();

	if (NODE_UNPARSERS[node_id] !== undefined) {
		return NODE_UNPARSERS[node_id];
	}

	var unparser = (new Unparser()).setNode(node);

	NODE_UNPARSERS[node_id] = unparser;
...
class Unparser {
	...
	getChildren() {
		if (!this.children) {
			...
			this.children = child_nodes.map(getUnparserForNode);
		}
		...
		if (typeof callback_or_map !== 'function') {
			...
			callback_or_map = function callback(rule_name, index) {
				...
				if (Array.isArray(rule_results)) {
					let result = rule_results[index];

					// NOTICE: We transform undefined values into null values
					// when we fail to retrieve a result by index (ie, we
					// exceed the bounds of the array we were supplied). This
					// is because, if we were to return `undefined` from here,
					// Heket would subsequently try to substitute the value
					// for the rule in question using its fixed value, if one
					// exists. Returning null prevents this behavior.
					if (result === undefined) {
						return null;
					}
	...
	unparse(callback_or_map) {
		...
		throw new Error('Unable to unparse value; node was in invalid state');
	}
	...
	unparseViaRuleName(callback) {
		...
		if (typeof rule_value === 'object') {
			return Unparser.fromRule(rule).unparse(rule_value);
		}
	...
	unparseViaAlternativeChildren(callback) {
		...
		throw new NoMatchingAlternativeError(null, this.getNode());
	}
 ...
 * @returns {lib/unparser}
 */
Unparser.fromNode = function fromNode(node) {
	return getUnparserForNode(node);
};
 ...
 * @returns {lib/unparser}
 */
Unparser.fromRule = function fromRule(rule) {
	return Unparser.fromNode(rule.getNode());
};
