...
class Match {
	...
	buildResult() {
		...
		if (this.rootNodeHasChildren()) {
			this.getChildrenOfRootNode().forEach(function each(child_node) {
				if (!child_node.hasRuleName()) {
					return;
				}

				var rule_name = child_node.getRuleName();

				rules[rule_name] = this.getResultForNode(child_node);
			}, this);
		}
