...
class Matcher {
	...
	matchStringViaRuleName(string) {
		...
		if (!match) {
			console.log('- MISMATCH via rule "' + this.getRuleName() + '" for input: ' + string);
			return null;
		} else {
			console.log('+ MATCH!   via rule "' + this.getRuleName() + '" for input: ' + string);
		}
	...
	matchStringViaRepetition(string) {
		...
		while (this.getRepeatCount() < this.getMaxRepeats()) {
			...
			console.log('after adding result, child had: ' + child.results.length + ' results');
	...
	buildRulesFromResults() {
		...
		this.iterateOverResults(function iterate(child, result) {
			if (child.hasRuleName() || result.rule_name) {
				rules.push(result);
			} else {
	...
	resetResults() {
		console.log('resetting results for node:');
		this.printNode();
		console.trace();
		console.log(' ');
		console.log(' ');
