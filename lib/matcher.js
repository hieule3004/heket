...
class Matcher {
	...
	getChildForNode(node) {
		...
		if (!children[node_id]) {
			// Create the child matcher if it doesn't exist yet.
			children[node_id] = this.createMatcherForChildNode(node);
		}
	...
	getPriorChildWithRemainingAlternatives() {
		var index = this.getChildIndex() + 1;
	...
	hasRemainingAlternatives() {
		...
		if (this.hasRuleMatcher()) {
			return this.getRuleMatcher().hasRemainingAlternatives();
		}

		if (this.isOptional() && this.getChildNodes().length === 1) {
			return this.getFirstChild().hasRemainingAlternatives();
		}
	...
	matchString(string, allow_partial = false, bypass_regex = false) {
		var result;

		if (bypass_regex === false && this.canMatchViaRegex()) {
			return this.matchStringViaRegex(string, allow_partial);
		}
	...
	matchStringViaRegex(string, allow_partial) {
		var
			rule  = this.getRule(),
			regex = null;
		...
		try {
			...
			return this.matchString(string, allow_partial, true);
		}

		var regex_match = regex.exec(string);
		...
		var
			rule_names = rule.getRegexGroupRuleNames(),
			index      = 0;

		while (index < rule_names.length) {
			let
				rule_value = regex_match[index + 1],
				rule_name  = rule_names[index];
	...
	matchStringViaRuleName(string) {
		...
		try {
			match = matcher.matchString(string, allow_partial);
		} catch (error) {
	...
	hasRuleMatcher() {
		...
		return this.rule_matcher !== null;
	}

	getRuleMatcher() {
		if (!this.rule_matcher) {
			this.rule_matcher = (new Matcher())
				.setRule(this.getRuleFromNode());
		}

		return this.rule_matcher;
	}
	...
	matchStringViaAlternativeChildren(string) {
		...
		while (this.getChildIndex() < this.getChildCount()) {
			...
			try {
				...
				// We don't want to suppress instances of missing rule errors:
				if (error instanceof RuleNotFoundError) {
					throw error;
				}

				// We also don't want to suppress any errors outside of those
				// that we expect Heket to throw during the normal course of
				// string matching:
				if (!(error instanceof BaseError)) {
					throw error;
				}

				// Every other error, we can just drop it on the floor.
			}
	...
	moveToNextAlternative() {
		...
		if (this.isRepeating() && this.canDecrementMaxRepeats()) {
			...
			this.resetRepeatCount(false);
			return this;
		}

		if (this.hasRuleMatcher()) {
			this.getRuleMatcher().moveToNextAlternative().resetChildIndex();
			return this;
		}

		if (this.isOptional() && this.getChildNodes().length === 1) {
			this.getFirstChild().moveToNextAlternative();
			return this;
		}
	...
	resetChildIndex() {
		...
		if (this.hasRuleMatcher()) {
			this.getRuleMatcher().resetChildIndex();
		}

		return this.setChildIndex(0);
	}

	resetRepeatCount(reset_max_repeats) {
		this.iterateOverChildren(function iterate(child) {
			child.resetRepeatCount(reset_max_repeats);
		});

		if (this.hasRuleMatcher()) {
			this.getRuleMatcher().resetRepeatCount(reset_max_repeats);
		}

		this.setRepeatCount(0);

		if (reset_max_repeats === true) {
